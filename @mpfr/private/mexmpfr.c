...
void mx_to_mpfr(mpfr_t x, mx_mpfr_ptr pa, int idx) {
  ...
  mpfr_setsign(x, x, (mpfr_sign_t)pa[SIGN_NR][idx],
               MPFR_RNDN); /* set sign, rounding mode MPFR_RNDN is irrelevant */
   ...
   * for doing this.*/
  i_max = (int)(pa[D_END_NR][idx])-d_end_pred(pa,
          idx)-1; /* length of the limb array -1*/
  for(i=0; i<=i_max; ++i) {
...
void mpfr_to_mx(mx_mpfr_arr pmxa, mx_mpfr_ptr pa, int idx, mpfr_t x) {
  ...
  if((pa[D_END_NR][idx]+1) > mxGetNumberOfElements(pmxa[D_NR])) {
    pa[D_NR] = (double*)mxRealloc(pa[D_NR],
                                  (pa[D_END_NR][idx]+1)*sizeof(
                                    double)); /* allocate additional memory for new limb array */
...
void mex_mpfr_init(mx_mpfr_arr pmxa, mx_mpfr_ptr pa, mwSize ndim,
  ...
  default_nlimbs = ceil(((double)
                         default_prec)/GMP_NUMB_BITS); /* default number of limbs*/
  pmxa[D_NR] = mxCreateDoubleMatrix(1,
                                    default_nlimbs*mxGetNumberOfElements(pmxa[PREC_NR]), mxREAL);
...
void mex_plhs_set(mxArray* pl, mx_mpfr_arr pmxa, mx_mpfr_ptr pa) {
  ...
  if( nlimb < mxGetNumberOfElements(pmxa[D_NR])) {
    pa[D_NR] = (double*)mxRealloc(pa[D_NR],
                                  nlimb*sizeof(double)); /* shrink memory for limb array to the needed amount */
    mxSetPr(pmxa[D_NR], pa[D_NR]);
  }
