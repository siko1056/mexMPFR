...
{   
  ...
  mx_mpfr_arr pmxa;
  mx_mpfr_ptr pa;
  int (*fp)(mpfr_t rop, mpfr_rnd_t rnd) = NULL;   /* function pointer to a comparsion function*/
  
  rnd = (mpfr_rnd_t)mxGetScalar(prhs[0]); /* get first input parameter rounding mode */
  prec = (mpfr_prec_t)mxGetScalar(prhs[1]); /* get second input parameter precision */ 
  const_nr = (int)mxGetScalar(prhs[2]); /* get third input parameter identifier of the constant */
