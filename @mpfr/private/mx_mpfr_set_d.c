...
{   
  ...
  rnd = (mpfr_rnd_t)mxGetScalar(prhs[1]); /* get second input parameter rounding mode */
  prec = (mpfr_prec_t)mxGetScalar(prhs[2]); /* get third input parameter precision */
  ...
  idx_max=(mwIndex)mxGetNumberOfElements(prhs[0])-1;  /* idx_max := prod(dims)-1 */
  pind = mxGetPr(prhs[0]);
  for(idx=0;idx<=idx_max;++idx){        
    ternary = mpfr_set_d(rop,(double)(pind[idx]),rnd); /* store double value x in rop */
