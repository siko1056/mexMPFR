...
{   
  ...
  rnd = (mpfr_rnd_t)mxGetScalar(prhs[2]); /* get second input parameter rounding mode */
  prec = (mpfr_prec_t)mxGetScalar(prhs[3]); /* get third input parameter precision */
  ...
  for(idx=0;idx<=idx_max;++idx){  
    ...
    ternary = mpfr_set_str(rop,str,base,rnd); /* convert numeric string to mpfr-format */
