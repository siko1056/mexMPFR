    ...
    for j=1:s(2)
      for i=1:s(1)
        if c_str(i,j).mantissa(1) == '-'
          disp ([inputname(1), '(', num2str(i), ',', num2str(j), ') = -0.', ...
            c_str(i,j).mantissa(2:end),' E',num2str(c_str(i,j).exp)])
        else
          disp ([inputname(1), '(', num2str(i), ',', num2str(j), ') = 0.', ...
            c_str(i,j).mantissa,' E',num2str(c_str(i,j).exp)])
        end
      end
    end
  else
