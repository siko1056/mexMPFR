...
switch nargin
    ...
    case 3
        ...
        if ~isnumeric(base) || round(base)~=base || base<2 || base>62
           error('invalid call of mpfr constructor with wrong format of struct input. Base must be an integer from the interval [2,62]. ')
        end        
    otherwise
        error('invalid call of mpfr constructor')
end
...
switch class(x)
    ...
    case 'char'
        CONST_LOG2_NR    = 0 ; % ln(2)
        CONST_PI_NR      = 1 ; % pi 
        CONST_EULER_NR   = 2 ; % Euler constant 
        CONST_CATALAN_NR = 3 ; % CATALAN_NR
    ...
    %        2.1          2.2     2.385693456
        ...
        for i=1:numel(x)
            if ~ischar(x{i})
               error(['invalid call of mpfr constructor: ',num2str(i),'-th component of input is not a string.'])
            end
        end
